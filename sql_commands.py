...
def filter_data(table_name, column_names: tuple, connection_to_db, data_to_filter, column_name):
    cursor = connection_to_db.cursor()
    cursor.execute(f"SELECT {column_names} FROM {table_name} WHERE {column_name} = ?", data_to_filter)
